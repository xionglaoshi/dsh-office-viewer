...
**本插件不使用懒加载 / 虚拟滚动**（`lazySlides: false, lazyMedia: false`）。

理由：预览的目的是让用户看到**产物的真实全貌** —— 页数、末尾内容、有没有缺失，
都必须一眼可见。懒加载会把"没渲染出来"和"本来就没有"变成同一种表象，
**让用户无法判断产物是否完整** —— 而这正是预览要回答的问题。

这不是理论洁癖，是踩过坑的：

> 2026-09-20，集成时给 pptx 引擎传了 `lazySlides: true`（本意省内存）。
> 结果一份 **13 页**的 pptx **只渲染出 5 页**，且失败得极其隐蔽 ——
> DOM 里 14 个 slot 都在、页号也对（`data-slide-number="6"`）、
> 滚动也救不回来、**控制台无任何报错**。
>
> | 设置 | 有内容页数 |
> |---|---|
...
大文件首屏会更慢、DOM 会更大 —— 这个代价本插件选择接受。
若确有超大文件压力，正确做法是给用户明确的"内容过大"提示，而不是静默地少渲染。

**自测也据此加固**：`scripts/selftest.mjs` 会统计"有页号但无内容"的空页壳
（`.flyfish-pptx-slide-slot`），发现空页即判失败 —— 而不是像最初那样只看
"有没有渲染出东西"。详见 [DESIGN.md](./DESIGN.md) §6。
