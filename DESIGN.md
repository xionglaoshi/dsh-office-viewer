...
这条不是性能偏好，是**功能定义**：用户要看的是自己产物的真实全貌 ——
页数、末尾内容、有没有渲染缺失，都必须一眼可见。懒加载会把"没渲染"和
"本来就没有"混成同一种表象，**让用户无法判断产物是否完整**，这正是预览要回答的问题。

### 2026-09-20 的实测事故（本原则的由来）

集成时我给 pptx 引擎传了 `lazySlides: true`（本意是省内存）。结果：

一份 **13 页**的 pptx **只渲染出 5 页**，且失败得极其隐蔽：

- DOM 里 14 个 slot（13 页 + 1 尾部占位）**都在**，页号也对（`data-slide-number="6"`）
- 空页只是空壳：`<div class="flyfish-pptx-slide-slot" ... style="min-height:770px"></div>`
- 逐页 `scrollIntoView()` **也救不回来**
- 控制台**无任何报错、无警告** —— 安静地少渲染

对比实验（同一文件、同样等待 25s）：

| 设置 | 有内容页数 |
|---|---|
| `lazySlides: true` | **5 / 14** ❌ |
| `lazySlides: false` | **14 / 14** ✅ |

修复：`src/engines/pptx.js` 固定 `lazySlides: false, lazyMedia: false`。

### 对本插件的约束

1. **不得为省资源开启任何懒加载/虚拟滚动开关**，包括但不限于
   `lazySlides`、`lazyMedia`、以及引擎未来可能新增的同类参数。
2. 大文件首屏更慢、DOM 更大 —— **这个代价必须接受**。
   若确有超大文件压力，正确做法是给用户一个明确的"内容过大"提示，
   而不是静默地少渲染。
3. **自测必须校验"页数/条目数"，不能只断言"渲染出了东西"。**
   本次事故之所以逃过原有自测，就是因为那只检查 `textLen > 0`。
   现已改为对照独立来源（LibreOffice 转 PDF 页数、`slideN.xml` 个数）校验数量。
